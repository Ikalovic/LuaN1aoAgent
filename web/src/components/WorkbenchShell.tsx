...
export function WorkbenchShell({ view, wallboardUrl, onViewChange, onHome, canApprove, canManageCredentials = false, pendingCount, toolbar, children, inspector, onCloseInspector }: { view: ViewKey; wallboardUrl?: string; onViewChange: (view: ViewKey) => void; onHome?: () => void; canApprove: boolean; canManageCredentials?: boolean; pendingCount: number; toolbar: ReactNode; children: ReactNode; inspector?: ReactNode; onCloseInspector: () => void }) {
  ...
  const tabs = (group?.views ?? []).filter((key) => (key !== "approvals" || canApprove) && (key !== "credentials" || canManageCredentials));
