...
- **Closest existing:** `whois` is in `EXECUTOR_TOOL_PROBE_LIST` (`executor-environment.ts:28`).
- **Insufficient because:** (a) it is only a *probe string* — the executor image (`executor-image/Dockerfile`) does **not** install `whois`, and the image's authoritative tool label lists `["bash","chromium","curl","dig","git","imagemagick","ip","iptables","jq","nc","nmap","node","openssl","python3","tesseract","wget"]` — no `whois`. So the probe list advertises a tool the container cannot run. That inconsistency is itself a defect worth fixing before building on it. (b) Even if present it is unstructured text, with no schema, no caching, no rate-limit handling, and no registrant-redaction awareness.
...
### 10.15 The tool-probe list advertises tools the image does not ship
- **Evidence:** `EXECUTOR_TOOL_PROBE_LIST` (`executor-environment.ts:25-30`) lists `whois, hydra, john, hashcat, gdb, objdump, sqlmap, go, npm, gcc, make, unzip, tar, pip, ncat, strings, file, xxd, base64, python`; the image label (`executor-image/Dockerfile`) advertises only `bash, chromium, curl, dig, git, imagemagick, ip, iptables, jq, nc, nmap, node, openssl, python3, tesseract, wget`.
- **Two distinct mechanisms, worth separating:** for **Docker** mode the facts block reads the *image label* (`inspectDockerImageTools`, `:113-141`) and requires `version === 1 && uid === 1000 && rawSockets === false`; the probe list is used only for the shell loop in **host** modes (`probeShellLoop`, `:143-145`) and as the result filter in both. So in Docker the agent is told the truth (16 tools), while the probe list remains a stale union. Any recon design must treat the **image label as the authority** and, if a new tool is needed, add it to the Dockerfile rather than to the probe list.
- **Consequence for recon:** the image has no `whois`, no `dnsx`, no `httpx`, no `gobuster`, no `ffuf`, no `nikto`, no `whatweb`, no `wafw00f`, no `openssl s_client` wrapper (openssl is present but unwrapped), and no Python HTTP/JSON libraries beyond stdlib. `nmap` + `dig` + `curl` + `python3` + `chromium` is the entire recon toolbox.
