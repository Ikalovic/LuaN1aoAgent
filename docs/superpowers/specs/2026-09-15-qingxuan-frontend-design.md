...
| QX-WALL-01 / 大屏展示成稿必需 | 精密舞台环境背景，不承载数据 | 3840×2160，16:9，无文字，中部及两侧阅读区低细节 | 1080p WebP <=450KB、4K <=1MB；`web/public/art/wallboard-surface-1080.webp`、`wallboard-surface-4k.webp` |
...
初版完成了 Markdown、代码围栏、本地链接和占位检查。补充大屏需求时已同步上游并通过 `npm run build`、4 个相关前端测试文件的 23 项测试，以及 5 个服务端测试文件的 17 项测试；这些验证针对更新后的代码基线，不代表前端重构或大屏已经实现、通过验收。未生成图片。
