...
独立子规格：[态势大屏设计](2026-09-15-qingxuan-wallboard-design.md)。主规格负责操作工作台，大屏子规格负责远距离展示；二者复用品牌、数据口径和基础图形能力。
...
工作台顶栏以 Monitor 图标打开同一运行的独立大屏。其布局以中央资产拓扑为主，两侧展示任务/角色活动和重点发现，底部为事件趋势，青玄品牌、运行状态与数据时间持续可见。支持 1920×1080、4K 和标准/远距字号预设；全屏与暂停更新是展示控制，不改变后端运行。
...
| QX-WALL-01 / 可选 | 独立大屏低对比度背景材质，不承载数据 | 3840×2160，16:9，无文字，中央低细节 | 1080p/4K WebP/AVIF，`web/public/art/wallboard-surface-1080.webp`、`wallboard-surface-4k.webp`；提示词与交付要求见 [大屏规格第 9 节](2026-09-15-qingxuan-wallboard-design.md#9-图片与美术资源) |
...
工作台总览背景采用纯色或低对比度 CSS 网格，不需要背景大图。独立大屏可使用 QX-WALL-01，没有该素材时同样用纯色。图谱节点和边不需要额外美术贴图。所有数据与文字保持实时绘制。
...
| 5. 独立态势大屏 | wallboard 路由、三列展示、主图、只读控制、全屏与暂停 | 1080p/4K 展示、大屏数据口径与异常状态通过验证；依赖阶段 1–3，可独立编写实施计划 |
...
| 独立大屏 | URL 直达、1080p/4K、全屏拒绝回退、暂停不停止运行、延迟状态常驻、只读权限符合子规格 |
