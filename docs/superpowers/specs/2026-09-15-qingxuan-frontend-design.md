...
| 管理 | Skills / MCP / 审批 | `skills` / `mcp` / `approvals`，保留 | 全局工具能力、MCP 与人工审批；环境配置从 MCP 页打开 |
...
MCP 页面保留名称/描述搜索、启用/禁用/未配置筛选、工具列表、配置状态、诊断与刷新。管理员按现有 UI 条件启停，分析员只读。环境配置继续从 MCP 页进入 EnvConfigDrawer，仅 admin 可请求 `/api/env`；保留新增、编辑、移除、恢复、放弃和一次提交 set/remove 的流程。敏感值仅显示服务端 preview，编辑默认空值，只有明确输入新值才替换；失败保留修改，不将掩码写回原值。保存后刷新 MCP 状态，生效范围沿用现有服务说明，不声称运行中所有 Agent 立即热更新。
