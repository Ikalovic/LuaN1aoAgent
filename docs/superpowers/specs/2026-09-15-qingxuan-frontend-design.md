...
日期：2026-09-15。初版基线：`d7da7e1`；当前基线：`597509c`，已合入远端 `60a1753`。

状态：设计文档完成，供实施前审阅。用户已选择深色安全指挥工作台，明确产品名称与图片资源记录要求，并新增独立展示大屏需求；其余细节为本文提出的具体方案，不代表已实现或已逐项确认。

本次使用 Superpowers 的 brainstorming 方法收敛需求、比较方向、定义边界并进行审阅。本文是设计规格；writing-plans 的逐任务实施计划应在设计审阅后另行编写。设计阶段未实现前端；用户后续已要求生成素材，现已交付图片与预览，见 [美术交付说明](../../qingxuan-art-delivery.md)。

独立子规格：[态势大屏设计](2026-09-15-qingxuan-wallboard-design.md)。主规格负责操作工作台，大屏子规格负责远距离展示；二者复用品牌、数据口径和图数据投影。大屏第二版根据用户“更炫酷”的反馈，提出 Three.js 立体态势舞台，常规工作台仍使用 Cytoscape。
...
| 独立展示大屏 | 面向会议、展示与持续观察 | 需专门布局、展示控制和大屏验收 | 按用户追加要求纳入，作为同应用独立只读页面 |
...
- FlyFish 实际查看官方大屏 GIF 多个时刻与组件缩略图，借鉴中央立体场景、切角页头和外围信息组织；其在线 demo 返回 502，未验证在线交互。春秋 CCB2026 的赛事/观赛公开入口本次仅显示背景，不能声称已看到其观赛主体。具体来源和访问边界见调研记录及大屏子规格。

详细调研见 [前端重构准备调研](../../frontend-refactor-research.md)。租户地址、抓包认证信息和产品实访截图不进入发布资源。

## 3. 范围与交付边界

本次设计覆盖统一品牌与双主题、导航和运行选择、态势总览、独立态势大屏、三图、执行轨迹、HTTP 流量、连接、报告产物、Skills、MCP 管理、环境配置、Agent 详情、审批、登录和任务启动/继续流程。

本次上游同步新增 MCP 注册表与配置管理（`17bc43b`）、Agent 详情抽屉（`60a1753`）。它们属于既有功能保留范围，重构不能移除对应入口、接口、权限与跳转行为。

第一期允许的后端改动仅限状态响应的数据覆盖元信息与必要的读取状态区分。业务控制器、调度逻辑、网络执行与权限模型沿用现有实现。前端补齐已有接口字段，修复与导航及展示直接相关的契约缺口。

本期不承诺：全量历史统计、跨运行资产归并、历史图回放、统一 CVSS 风险评分、资产地理定位、外部攻击监测、3D 地球、新的 AI 分析服务和案例管理系统。数据未具备时不以模拟曲线、随机数字或装饰攻击线补齐界面。

独立大屏的立体资产舞台不属于 3D 地球：仅将已有图投影成可交互空间场景，坐标/高度没有地理或风险数值含义。仅大屏允许独立的装饰环境运动，不能伪装成真实执行事件。

## 4. 信息架构

所有运行相关视图以“当前运行”为上下文，Skills、MCP、环境配置与审批属于全局管理。默认入口从轨迹改为态势总览；已有带 `view` 的链接继续有效。
...
| 管理 | Skills / MCP / 环境变量 / 凭证管理 / 审批 | `skills` / `mcp` / `env` / `credentials` / `approvals` | 环境变量为独立全局管理页，保留 MCP 内快捷入口；凭证沿用当前运行上下文 |
| 顶栏展示入口 | 态势大屏 | `wallboard`，新增 | 独立全屏布局，不渲染普通导航和检查器 |
...
工作台顶栏以 Monitor 图标打开同一运行的独立大屏。第二版采用“立体态势舞台”：居中青玄品牌和分层切角页头，中央 Three.js 斜俯视资产节点与真实关系，两侧任务/角色活动和重点发现，底部事件趋势、焦点结论及近期事件。品牌、运行状态与数据时间持续可见。支持 1920×1080、4K、标准/远距排版；全屏与暂停仅改变展示。

普通工作台与推理/任务图继续使用 Cytoscape；大屏资产主场景单独增加 Three.js，使用共享 graph.ts/ELK 投影。WebGL 失败、低性能或窄屏回退平面。事件动效依赖真实引用，首次加载和恢复不补播旧事件；视觉目标、帧率预算与验收以子规格为准。

大屏不显示任务变更、审批操作、流量重放、MCP 开关和环境配置。具体布局、刷新状态、URL 与资源交付见 [独立大屏规格](2026-09-15-qingxuan-wallboard-design.md)。
...
- 工作台动效以 120–180ms 的颜色/透明度过渡为主；尊重 `prefers-reduced-motion`，不持续播放雷达扫描、流光边框或无依据的连线动画。独立大屏的入场、环境轨道和事件聚焦按子规格第 4 节执行，不扩散到日常表格和操作页面。
...
下方展示任务图状态分布、四种角色的最近事件和已加载事件柱状图。角色区域标记“最近活动”，不称在线 Agent 心跳。图表使用惰性加载的 Ant Design Charts Column；[官方组件清单](https://ant-design-charts.antgroup.com/en/components/overview)提供成熟统计图能力。该依赖只用于趋势图；工作台关系图继续使用 Cytoscape，Three.js 只用于独立大屏资产舞台。
...
保留上游新增 AgentDetailDrawer：从总览角色区域或执行页角色入口打开，查看最近工作、决策、动作、观察和近期记录。当前事件单列，后续最多 30 条活动；点击记录关闭抽屉并定位执行轨迹，同时设置相应角色筛选。事件数仍属已加载范围，最近事件状态不升级为真实在线心跳；derived intent 的文案规则继续生效。
...
MCP 页面保留名称/描述搜索、启用/禁用/未配置筛选、工具列表、配置状态、诊断与刷新。管理员按现有 UI 条件启停，分析员只读。环境变量新增独立 `view=env` 页签，与 MCP 内 EnvConfigDrawer 复用同一编辑器，仅 admin 可挂载编辑器并请求 `/api/env`；保留新增、编辑、移除、恢复、放弃和一次提交 set/remove 的流程。独立页不依赖运行加载，采用全宽表格、变量名搜索和底部提交栏，兼容深浅主题与窄屏表格内滚动。已保存敏感值仅显示服务端 preview，敏感值编辑默认空值，新变量与敏感修改在待保存状态同样掩码；只有明确输入新值才替换。失败保留修改，不将掩码写回原值；未保存离页需确认，放弃操作清空输入草稿。在 MCP 内保存后刷新 MCP 状态，生效范围沿用现有服务说明，不声称运行中所有 Agent 立即热更新。本次不增加后端接口或图片资源，不读写真实配置做测试。
...
| MCP 查看 / 开关 | 查看及按现有 UI 条件变更 | 只读 |
| 环境配置读取与保存 | `admin:env` | 不请求接口、不显示入口 |
| 态势大屏 | 只读展示当前有权访问的运行 | 同样可查看，审批数据仍不可见 |

Skills 与 MCP 开关后端目前允许 operator:mutate，而 UI 限管理员；本次保留较窄的现有 UI 行为，另行记录权限契约差异，不在视觉重构中扩大操作入口。环境配置读取和写入均要求 admin:env。当前运行访问受角色与 RuntimePathPolicy 根目录限制，导航不暗示已有“仅我的项目”隔离。
...
保持 `runtimeDir` 和旧 `view`，包括上游新增的 mcp；新增 overview/findings/wallboard，并支持已出现的 `nodeId`。增加 `traceId`、`taskId`、`exchangeId`：nodeId 选中图节点或发现；traceId 选中轨迹；taskId 在任务图定位该任务、在轨迹/报告/流量作为任务过滤；exchangeId 以不透明字符串经 URL 编码透传，不假定为整数。
...
| `web/src/components/WallboardView.tsx`、`WallboardControls.tsx` | 独立展示布局与全屏/暂停等展示控制，详见大屏子规格 |
| `web/src/components/WallboardScene.tsx`、`wallboardScene.ts`、`useWallboardPresentation.ts` | 仅大屏：Three.js 场景、共用图投影的空间表达、真实事件展示基线与动效；按需加载，低性能回退 Cytoscape |
| 现有 `AgentDetailDrawer.tsx` | 保留角色活动详情及跳转执行轨迹，接入新入口和主题 |
| 现有 `McpView.tsx`、`EnvConfigDrawer.tsx` | 保留新上游管理功能，统一视觉并遵循原权限与编辑状态 |
| 现有 `App.tsx` | 组合认证后工作区与业务行为，逐步移出布局和导航细节 |
| 现有各业务 View | 保留领域交互，接入统一主题、导航和状态 |
| `web/src/types.ts`、`api.ts` | coverage、流量筛选和 historical 模式契约；保留 MCP 与环境配置契约 |
...
MCP 与环境配置独立于当前运行加载；大屏不挂载这些管理组件，不因为进入展示页面而获取环境配置。大屏与工作台在同一个标签页互斥渲染，避免同时维护两套运行轮询。

审批页可见时按现有有待办 2 秒、无待办 5 秒刷新，响应同时更新全局角标，并暂停外壳的独立轮询；离开后角标恢复 15 秒刷新。数据获取是轮询，不在产品中声称毫秒级实时推送。

## 12. 图片与美术资源清单

本节保留美术目标规格。已完成 6 类素材导出，后续采用用户提供的 3840×2160 大屏背景新增 V2，当前共 27 个发布文件，尚未接入业务页面。当前大屏 4K 版本没有本地放大；其他素材仍存在裁切、缩放及母版尺寸差异。实际尺寸、加工过程和预览以 [美术交付说明](../../qingxuan-art-delivery.md) 为准，不能将目标规格当作模型原生输出记录。资源缺失时仍使用第 12.4 节降级方案。
...
| QX-WALL-01 / 大屏展示成稿必需 | 精密舞台环境背景，不承载数据 | 3840×2160，16:9，无文字，中部及两侧阅读区低细节 | 1080p WebP <=450KB、4K <=1MB；`web/public/art/wallboard-surface-v2-1080.webp`、`wallboard-surface-v2-4k.webp` |
| QX-WALL-02 / 大屏展示成稿必需 | 切角分层页头两翼装饰，不含品牌文字 | 3840×320，12:1，透明 PNG；中央 40% 透明留白 | 1920×160 透明 WebP <=180KB；`web/public/art/wallboard-header-rail.webp`，PNG 回退 |
| QX-WALL-03 / 可选精修 | 立体地台的无缝中性金属底色纹理 | 1024×1024，正视无透视、无烘焙光源 | WebP <=200KB；`web/public/art/wallboard-deck-albedo.webp` |

大屏三项的详细生成提示词、裁切、缺图回退与预算见 [大屏规格第 9 节](2026-09-15-qingxuan-wallboard-design.md#9-图片与美术资源)。成稿必需不阻塞功能开发，但不能以纯色回退冒充美术交付完成。
...
| 工作台资产拓扑、证据链、任务图 | Cytoscape + ELK 与真实 API 数据 | 不使用生成图片充当可交互图谱 |
| 大屏立体资产舞台 | Three.js + 共享 graph.ts/ELK 投影 | 节点底座、关系与状态实时绘制；图片仅用作环境和材质，不复制业务计算 |
| 活动趋势与任务分布 | Ant Design Charts / Ant Design Progress 与真实统计 | 不用静态图片或随机数据模拟实时曲线 |
| 报告附件图片 | 原始 Artifact，沿用现有读取校验 | 不用美术图片替代证据截图 |

工作台总览背景采用纯色或低对比度 CSS 网格，不需要背景大图。独立大屏使用 QX-WALL-01/02 形成完整舞台环境，QX-WALL-03 精修地台材质；缺图时纯色、CSS 细线和程序材质维持功能可用。普通图谱节点与边无需美术贴图，所有数据和文字保持实时绘制。
...
| 4. 工作流迁移 | 轨迹、Agent 详情、流量、连接、报告、Skills、MCP、环境配置、审批、登录与任务表单 | 核心操作和角色权限无回归，移动端可完成主要动作 |
| 5. 独立态势大屏 | wallboard 路由、展示页头、立体资产舞台、事件聚焦、二维回退、只读控制、全屏与暂停 | 1080p/4K 视觉审阅、数据和异常、WebGL 降级与帧率验证通过；依赖阶段 1–3，可独立编写实施计划 |
| 6. 美术接入与验收 | 已交付图片、可访问性、响应式和性能检查 | 资源、数据、交互与视觉验收逐项通过 |

每阶段保持可运行；由阶段范围建立独立可审阅的提交。品牌素材可在阶段 1 使用文本降级，阶段 6 再接入；不等待美术产出才启动功能开发。
...
| 上游新增流程 | MCP 筛选/开关、配置诊断、admin 环境配置掩码与 set/remove、Agent 详情和轨迹定位无回归 |
| 独立大屏 | URL 直达、1080p/4K 立体舞台与视觉审阅、事件去重、WebGL/性能降级、全屏拒绝回退、暂停不停止运行、延迟状态常驻、只读权限符合子规格 |

现有测试基础：`web/src/App.test.tsx`、`graph.test.ts`、各业务组件测试及 `test/web-server-*.test.ts`，并纳入上游 AgentDetailDrawer、McpView、EnvConfigDrawer 测试。新增测试重点是 situation 派生口径、coverage、导航历史、异步隔离、大屏暂停/全屏与关键权限；不为静态配色或单纯文案写脆弱快照测试。
...
性能验收以同机同数据的前后比较记录：基准数据为 1200 节点、2400 边、700 事件，覆盖持续轮询与 20 次运行切换。不得出现随刷新持续增长的 Canvas 实例、监听器或缓存；前后记录首个内容出现时间与交互响应，若回归超过 20% 则分析并修复或说明批准的取舍。Ant Design Charts 不进入登录页首包，Three.js 与大屏模块不进入登录/工作台首包；立体舞台的可见节点上限和独立性能预算见子规格。

## 15. 设计结论与审阅重点

已确定的产品方向：青玄 自动渗透agent、默认深色安全指挥工作台、保留浅色主题、增加独立展示大屏、沿用 Ant Design 与现有图谱能力、真实数据支撑态势、记录后续 AI 美术资源。

本文给出第一期明确边界：当前运行快照与覆盖信息、以任务和证据为中心的分析流程、兼容旧入口及运行数据、有限后端增量。实施前审阅重点为第 5 节首屏布局、第 8 节数据口径、第 12 节美术资源规格和第 13 节阶段顺序。

大屏的空间分配、观看距离和控制行为独立审阅，以 [大屏子规格](2026-09-15-qingxuan-wallboard-design.md) 为准。上游 MCP、环境配置与 Agent 详情作为基线功能一并保留。

本版追加的立体舞台、展示动效与美术强化回应用户对初版视觉力度不足的反馈，是待用户审阅的具体方案，不意味着用户已经批准了 Three.js 等全部实现细节。
...
初版完成了 Markdown、代码围栏、本地链接和占位检查。补充大屏需求时已同步上游并通过 `npm run build`、4 个相关前端测试文件的 23 项测试，以及 5 个服务端测试文件的 17 项测试；这些验证针对更新后的代码基线，不代表前端重构或大屏已经实现、通过验收。当时未生成图片，后续素材生成交付与其独立验证见上文链接。
