...
- Design and plan committed as `537c3a9`; implementation committed as `8505607`. The user subsequently requested merging into local `main`; no remote push.
...
Merged with upstream `52db88a` credential management, retaining its administrator entry inside the redesigned workbench. Post-integration verification passed: 186 frontend tests, 35 focused backend tests, server and web builds, entry browser checks, and desktop/mobile credential layouts. Fixed the reproduced credential-store lock initialization ordering by setting the existing busy timeout before WAL mode. Unrelated local documents and running processes were preserved; no push or backend restart.
